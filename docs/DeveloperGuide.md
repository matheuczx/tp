...
| v2.1    | tutor                           | calculate total fees generated by a specific student for a given month                                      | determine the amount paid by the student without manually cross-referencing lesson schedules and calendars.                                 |
| v2.1    | tutor                           | aggregate total revenue across all active students for a specific month                                     | obtain a high-level overview of total monthly business performance and financial health.                                                    |
...
### View monthly student fees: `fee-summary`

- **Prerequisites:** Ensure at least one active student exists with an assigned lesson schedule. Use `list` to identify a student index.

- **Test case:** `fee-summary 1 ym/2026-04`
  - **Expected outcome:** Displays a report containing the student's name, the target month, and the total fee calculated based on the number of lessons in that month.

- **Test case:** `fee-summary 1`
  - **Expected outcome:** Error message shown; the `ym/` prefix is mandatory.

- **Other incorrect commands to try:** `fee-summary`, `fee-summary 99 ym/2026-04`, `fee-summary 1 ym/2026-13`
  - **Expected outcome:** Error message shown; verifies the system validates both index and month format.

---

### View monthly income: `monthly-income`

- **Prerequisites:** Ensure at least one active student has assigned lessons for the target month.

- **Test case:** `monthly-income ym/2026-04`
  - **Expected outcome:** Displays the Monthly Income Report, showing the target month and the grand total revenue aggregated across all active students.

- **Test case:** `monthly-income`
  - **Expected outcome:** Error message shown; the `ym/` prefix and date are mandatory.

- **Other incorrect commands to try:** `monthly-income ym/2026-13`, `monthly-income 2026-04`
  - **Expected outcome:** Error message shown; verifies the system strictly enforces the `ym/YYYY-MM` format.
